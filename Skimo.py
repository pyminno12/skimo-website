...
def load_user_db():
    try:
        ...
        # 💡 최초 실행 시 개발자님의 아이디 'PYM'을 최고 관리자(ADMIN)로 자동 등록합니다.
        initial_db = {
            "PYM": {"pw": "1234", "role": "ADMIN"},
    ...
    with tab2:
        ...
        if st.button("회원가입 신청", use_container_width=True):
            ...
            if not reg_id or not reg_pw:
                ...
                # 💡 회원가입 시 생성되는 모든 아이디에 개발자 관리 권한(ADMIN)을 무조건 부여하도록 세팅
                current_db[reg_id] = {"pw": reg_pw, "role": "ADMIN"}
                save_user_db(current_db)
                st.session_state.user_db = current_db
                st.success(f"🚀 개발자 관리자 계정({reg_id}) 가입 성공! 로그인 탭에서 로그인해 주세요.")
...
with c_menu:
    ...
    selected_menu_raw = st.selectbox("Menu", menu_list, index=st.session_state.menu_idx, label_visibility="collapsed")
...
with c_right:
    ...
    with sub_lang:
        st.selectbox("Language", list(LANG_DICT.keys()), index=0, label_visibility="collapsed")
    with sub_buttons:
...
if st.session_state.menu_idx == 0:
    ...
    col_text, col_video = st.columns([6, 6])
    with col_text:
    ...
    df = pd.DataFrame(data_list)[["BIB", "Name", "Team", "Category", "Status"]]
    ...
    if st.session_state.logged_in_user is None:
        ...
        if user_role not in ["ADMIN", "JUDGE"]:
            st.error("🚫 접근 거부: 귀하는 일반 사용자로, 패널을 조작할 권한이 없습니다.")
        else:
            # 👑 ADMIN 및 JUDGE 권한 진입 성공 영역
            st.success(f"✨ 시스템 접근 승인 완료 ({st.session_state.logged_in_user} | 권한: {user_role})")
            ...
            if st.button("🚨 데이터 필드 실시간 동기화"):
                ...
                st.success("데이터 필드가 정상적으로 동기화되었습니다.")
                time.sleep(0.5)
                st.rerun()

elif st.session_state.menu_idx == 4:
    st.markdown(f"## {T['menu'][4]}")
    st.write("공지사항 아카이브 영역입니다.")
